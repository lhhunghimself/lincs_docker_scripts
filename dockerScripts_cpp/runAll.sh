 ...
 then 
  ...
  echo "Required parameters are:
	...
	nThreads      -number of threads
        posFilterBits -number of bits for UMI position filtering -> leave out for gene level filtering
        "
 exit 1
fi
...
if [ -n $10 ]; then
 ...
 -v $configsDir:/root/LINCS/Configs\
 -v $paramsDir:/root/LINCS/Params\
  $container /root/LINCS/DockerScripts/run-alignment-analysis.sh $nWells $nThreads $10 > /$UMIdir/Results/log 2&>1"
